#!/bin/bash
###############################
#Name:SUT_001_SIT_BASEINFO_CHECK_RHEL_V001
#Author:yanshuo
#Revision:
#Version:V001
#Date:2017-12-12
#Tracelist:V001-->First Version
#Function: get base info under rhel
#No parameter!
#Usage:sh SUT_001_SIT_BASEINFO_CHECK_RHEL_V001.sh
#
###############################
...
log_file_name="${time_start}_SIT_BASEINFO_CHECK_RHEL.log"
current_path=`pwd`
...
echo `cat /etc/redhat-release |awk '{match($0,/([0-9]+\.[0-9]+)/,a);print a[1]}'`
...
echo `cat /etc/redhat-release|awk '{match($0,/([0-9]+\.[0-9]+)/,a);print a[1]}'` >> ${log_path}
...
fdisk -l|grep '^Disk /dev/'|sort|grep -v loop|grep -v ram 2>&1
fdisk -l|grep '^Disk /dev/'|sort|grep -v loop|grep -v ram >> ${log_path} 2>&1
chmod 777 tool/nvme_id_ctrl
#smart info
filter_disk=`fdisk -l|grep '^Disk /dev/'|sort|grep -v loop|grep -v ram|awk '{print $2}'|awk -F ":" '{print $1}' 2>&1`
...
if [ $? != 0 ]; then
echo -e "\033[31mipmitool is not installed! Please use yum -y install ipmitool to install it!! \033[0m"
echo "ipmitool is not installed! Please use yum -y install ipmitool to install it!" >> ${log_path}
exit 255
fi
