#!/bin/bash
###############################
#Name:SUT_001_SIT_BASEINFO_CHECK_DEBIAN_V001
#Author:yanshuo
#Revision:
#Version:V001
#Date:2017-12-20
#Tracelist:V001-->First Version
#Function: get base info under debian
#No parameter!
#Usage:sh SUT_001_SIT_BASEINFO_CHECK_DEBIAN_V001.sh
###############################
...
log_file_name="${time_start}_SIT_BASEINFO_CHECK_DEBIAN.log"
current_path=`pwd`
...
echo `uname -r|awk '{match($0,/([0-9]*\.[0-9]*)/,a);print a[1]}'`
#write info to log file
echo "Below is the OS Version!" >> ${log_path}
cat /etc/debian_version >> ${log_path}
echo "Below is the OS Kernel!" >> ${log_path}
uname -r|awk '{match($0,/([0-9]*\.[0-9]*)/,a);print a[1]}' >> ${log_path}
...
#hba_test=`find /sys -name port_name|grep  > /dev/null 2>&1`
if [ $? != 0 ]; then
...
if [ $? != 0 ];then
echo -e "\033[31mifconfig is not installed!Please use apt-get install net-tools to install it! \033[0m"
echo "ifconfig is not installed!Please use apt-get install net-tools to install it!" >> ${log_path}
exit 255
fi
...
if [ $? != 0 ]; then
echo -e "\033[31mlssci is not installed! Please wait while  install lsscsi! \033[0m"
echo "lssci is not installed! Please wait while instal lsscsi!" >> ${log_path}
cd tool
tar -zxf lsscsi-0.28.tgz
cd lsscsi-0.28/
...
which lsscsi > /dev/null
if [ $? != 0 ];then
echo -e "\033[31mlsscsi is installed failed! \033[0m"
echo "lsscsi is installed failed!" >> ${log_path}
exit 255
fi
fi
...
if [ $? != 0 ]; then
echo -e "\033[31mopenipmi service is not installed! Please insert disk 2 use apt-get install ipmitool to install it! \033[0m"
echo "openipmi service is not installed! Please use apt-get install ipmitool to install it!" >> ${log_path}
exit 255
else
modprobe ipmi_si
