...
#/usr/bin/dmesg |tail -n 20|grep -i ata|grep -i "link up"  >> sata_link_speed.txt
...
cat 1.txt >> plug.log
#link speed info
#echo "Below is the link speed for one cycle!" >> plug.log
#/usr/bin/dmesg |tail -n 20|grep -i ata|grep -i "link up" >> plug.log
#power cycle count
echo "Below is the power cycle count for one cycle!" >> plug.log
smartctl -a /dev/$DEV_name_in|grep Power_Cycle_Count >> plug.log
}
...
echo "Below is the baseline info !" > plug.log
cat baseline.xls >> plug.log
#link speed info
#echo "Below is the link speed start!" >> plug.log
#/usr/bin/dmesg |grep -i ata|grep -i "link up" >> plug.log
#power cycle count
echo "Below is the power cycle count start!" >> plug.log
smartctl -a /dev/sdb|grep Power_Cycle_Count >> plug.log
}
...
#date >> sata_link_speed.txt
#echo "sata_link_speed for one loop" >> sata_link_speed.txt
plug_in;
sleep 10
