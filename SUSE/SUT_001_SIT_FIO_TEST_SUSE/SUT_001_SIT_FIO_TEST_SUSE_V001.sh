#!/bin/bash
###############################
#Name:SUT_001_SIT_FIO_TEST_SUSE
#Author:yanshuo
#Revision:
#Version:v001
#Date:2017-12-18
#Tracelist:V001-->First Version
#Function: storage test under suse with fio
#Parameter_1: total test seconds
#Parameter_2(optional):system disk
#Usage:bash SUT_001_SIT_FIO_TEST_SUSE_V001.sh Parameter_1 Parameter_2(optional)
#Example:
#1:bash SUT_001_SIT_FIO_TEST_SUSE_V001.sh 3600 
#2:bash SUT_001_SIT_FIO_TEST_RHEL_V001.sh 3600 sda4
###############################
...
log_dir_name="${time_start}_SIT_FIO_TEST_SUSE"
log_file_name="${time_start}_SIT_FIO_TEST_SUSE.log"
current_path=`pwd`
...
#check fio  command
rpm -qa|grep libaio-dev > /dev/null 2>&1
if [ $? != 0 ];then
    echo -e "\033[31mPlease install libaio-devel to finish the preperation \033[0m"
    echo "Please use  install libaio-devel to finish the preperation" >> ${log_path}
    exit 255
fi
...
if [ $? != 0 ];then
	echo -e "\033[31mfio is installed failed!Please check if make/gcc is installed! \033[0m"
	echo "fio is installed failed!Please check if make/gcc is installed!" >> ${log_path}
	exit 255
fi
make > /dev/null 2>&1
if [ $? != 0 ];then
