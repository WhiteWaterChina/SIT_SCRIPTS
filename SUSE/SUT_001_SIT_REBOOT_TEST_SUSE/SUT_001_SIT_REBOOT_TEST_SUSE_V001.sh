#!/bin/bash
##############################
#Name:SUT_001_SIT_REBOOT_TEST_SUSE
#Author:Ward Yan
#Revision:
#Version:A01
#Date:2018-07-11
...
#Usage:bash SUT_001_SIT_REBOOT_TEST_SUSE_V001.sh Parameter_1 Parameter_2 Parameter_3　Parameter_4
#Example:
#1.reboot  bash SUT_001_SIT_REBOOT_TEST_SUSE_V001.sh reboot 43200 500 40
#2.dc/ac   bash SUT_001_SIT_REBOOT_TEST_SUSE_V001.sh dc 43200 500 40
###############################
...
log_dir_name="${time_start}_SIT_REBOOT_TEST_SUSE"
log_file_name="${time_start}_SIT_REBOOT_TEST_SUSE.log"
...
if [[ $confirm == "y" || $confirm == "Y" ]];then
  ...
  sleep $sleep_time
  echo "/bin/bash /home/reboot.sh &" >> /etc/rc.d/boot.local
  chmod 777 /etc/rc.d/boot.local
  if [ $reboot_type == "reboot" ];then
    reboot
  elif [ $reboot_type == "dc" ];then
    poweroff
