...
$DEV_name_out_temp=Get-EventLog -LogName System -InstanceId 2147942482 -ErrorAction SilentlyContinue
if ($DEV_name_out_temp)
{
$DEV_name_out_1=(($DEV_name_out_temp|Select-Object -ExpandProperty message).trim()).split("`r")[0] -match ".*?Adapter(\s#\d+\d*)*"
$DEV_name_out=$matches[0]
}
...
$DEV_name_in_temp=Get-EventLog -LogName System -InstanceId 1074200589 -ErrorAction SilentlyContinue
if ($DEV_name_in_temp)
{
$DEV_name_in_1=(($DEV_name_in_temp|Select-Object -ExpandProperty message).trim()).split("`r")[0] -match ".*?Adapter(\s#\d+\d*)*"
$DEV_name_in=$matches[0]
}
Start-Sleep -Seconds 1
...
#plug out test
Start-Sleep -Seconds 1
$temp_out_temp=(Get-EventLog -LogName System -InstanceId 2147942482 -ErrorAction SilentlyContinue)
if ($temp_out_temp)
{
$temp_out=((($temp_out_temp|Select-Object -ExpandProperty message).trim()).split("`r")[0])|
Select-String -Pattern "unplugged"
}
...
#plug in test!
Start-Sleep -Seconds 1
$temp_in_temp=Get-EventLog -LogName System -InstanceId 1074200589 -ErrorAction SilentlyContinue
if ($temp_in_temp)
{
$temp_in=(($temp_in_temp|Select-Object -ExpandProperty message).trim()).split("`r")[0]|
Select-String -Pattern "link is up"
#$temp_in
}
